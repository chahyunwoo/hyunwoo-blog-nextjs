...
<Callout type="info">
  이전 블로그 제작기를 통해 초기 설정 방법을 포스팅 했으므로, 자세한 내용은
  기술하지 않겠습니다.
</Callout>
...
혹은 해당 slug를 가진 파일을 찾아오기 위해 <strong style={{ color: "oklch(0.667 0.295 322.15)" }}>glob</strong> 을 사용했습니다.

그 후 <strong style={{ color: "oklch(0.667 0.295 322.15)" }}>gray-matter</strong>를 사용하여
...
이후 상세 페이지를 구현하기 위해, 앞서 설명했던 <strong style={{ color: "oklch(0.667 0.295 322.15)" }}>gray-matter</strong>를 통해
...
앞서 말씀드린 <strong style={{ color: "oklch(0.667 0.295 322.15)" }}>next-mdx-remote</strong> 를 사용하여 위 작업을 진행할 수 있습니다.

```tsx title="post-body.tsx" {8, 23}
