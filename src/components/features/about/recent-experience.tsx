  ...
  return (
    <AboutContainer title="RECENT EXPERIENCE">
      {profile.experience.map((experience, index) => (
        <Card key={experience.title} className={`bg-background ${index > 0 ? 'mt-4' : ''}`}>
          <CardHeader>
            <CardTitle className="flex flex-col gap-2">
              <p className="text-lg font-extrabold">{experience.title}</p>
              <span className="text-muted-foreground font-normal text-xs">{experience.period}</span>
            </CardTitle>
            <CardDescription>
              <h3 className="text-lg font-extrabold text-accent-foreground mt-2">{experience.role}</h3>
